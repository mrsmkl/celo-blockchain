...
func (c *core) handleTimeoutMsg() {
	...
	if lastProposal != nil && lastProposal.Number().Cmp(c.current.Sequence()) >= 0 {
		logger.Trace("round change timeout, catch up latest sequence", "number", lastProposal.Number().Uint64(), "cur_seq", c.current.Sequence(), "cur_round", c.current.Round(), "tag", "stateTransition")
		c.startNewRound(common.Big0)
	} else {
